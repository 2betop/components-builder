...
module.exports = function(files, callback) {
    files.forEach(function(info) {
        write(info.absolute, transform(read(info.absolute, 'utf8')));
    });
...
function transform(data) {
    ...
    estraverse.replace(ast, {
        leave: function(node) {
            ...
            } else if (isAMDCondition(node)) {
                return createCommonJsCondition(node.consequent);
            } else if (isRequire(node) && node.arguments[0].type == 'ArrayExpression') {
                return {
                    type: 'CallExpression',
                    callee: {
...
function createCommonJsCondition(obj) {
    return {
        type: 'IfStatement',
        consequent: obj,
        test: {
