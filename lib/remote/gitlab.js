...
function extendGitlab(instance) {
    ...
    instance.projects.repository.deleteFile = function(params, fn) {
        ...
        options.path = "projects/" + parseProjectId(params.projectId) + "/repository/files";
        options.data = params;

        delete params.projectId;

        requestAPI(options, function(error, ret) {
            fn(ret || true);
        });
    };
}
...
function requestAPI(options, callback) {
    ...
    request({
        ...
        uri: options.url + '/' + path.join(options.base_url, options.path) + '?private_token=' + options.token,
        json: true,
        body: options.data,
        headers: {
